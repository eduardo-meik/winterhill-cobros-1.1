-- FIX CRITICAL BUGS IN finalize_enrollment
-- 1. Fix ON CONFLICT to match actual constraint (student_id, guardian_id, numero_cuota)
-- 2. Add folio generation
-- 3. Return folio for receipt generation
    ...
    BEGIN
      ...
      LOOP
        ...
        ELSE
          ...
          -- FIXED: Use correct constraint columns
...
COMMENT ON FUNCTION public.finalize_enrollment(uuid, jsonb) IS 'Finalize enrollment: creates fees, generates folio. Use dry_run=true to preview. FIXED: ON CONFLICT and folio generation.';
