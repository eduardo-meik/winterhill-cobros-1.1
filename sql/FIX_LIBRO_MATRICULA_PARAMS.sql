-- ============================================================================
-- FIX: Libro de Matrícula - Manejo de parámetros vacíos
-- Fecha: 2025-12-22
-- ============================================================================
...
DECLARE
  ...
  v_status VARCHAR;
BEGIN
  -- Convertir parámetros TEXT a tipos correctos, manejando strings vacíos como NULL
  v_year := CASE
  ...
  WHERE 
    (
      ...
      OR (
        CASE
          WHEN c.year_academico IS NULL THEN NULL
          WHEN c.year_academico::text ~ '^[0-9]{4}$' THEN c.year_academico::text::int
          ELSE NULL
        END
      ) = v_year
    )
...
'Genera Libro de Matrícula con numeración correlativa. Parámetros: p_year (año), p_estado (estado estudiante), p_enrollment_status (estado enrollment). Todos opcionales - usar NULL para traer todos.';
