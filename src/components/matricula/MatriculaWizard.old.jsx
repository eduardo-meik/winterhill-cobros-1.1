/**
 * MatriculaWizard — Multi-step enrollment wizard.
 *
 * ⚠️  TECH DEBT (SRP violation — ~2,700 lines in a single component)
 * This component should be decomposed into:
 *   1. Sub-components per step:
 *      - GuardianStep, StudentStep, EconomicStep, DocumentStep, ReviewStep
 *   2. Custom hooks for business logic:
 *      - useEnrollmentWizard (step navigation, state machine)
 *      - useDocumentGeneration (PDF/HTML generation orchestration)
 *      - useEnrollmentPersistence (Supabase CRUD for enrollment records)
 *   3. Shared form state via useReducer or Zustand instead of 30+ useState hooks.
 *
 * Tracked in: AUDIT_REPORT.md — Fix #17
 */
...
import { buildEnrollmentReceiptHtml, generateEnrollmentReceiptPdf } from '../../services/enrollmentReceipt';
...
export function MatriculaWizard() {
  ...
  useEffect(() => {
    ...
    console.log('📊 Loading saved economic data from enrollment:', enrollment.id);
    ...
    const fetchCheques = async () => {
      try {
        ...
        if (dbCheques && dbCheques.length > 0) {
          console.log('📝 Loaded existing cheques count:', dbCheques.length);
  ...
  const handleSaveEconomic = async () => {
    ...
    console.log('💾 Guardando datos económicos para enrollment:', enrollment.id);
  ...
  const handleFinalizePreview = async () => {
    ...
    try {
      ...
      console.log('[FINALIZE_PREVIEW] enrollment:', enrollment.id);
  ...
  const handleGeneratePagare = async () => {
    if (!guardian || !enrollment) {
      console.error('❌ Missing guardian or enrollment:', { hasGuardian: !!guardian, hasEnrollment: !!enrollment });
      if (!guardian) {
    ...
    console.log('🎯 handleGeneratePagare started, guardian:', guardian?.id, 'students:', students?.length);
    ...
    console.log('💵 Economic numbers calculated for enrollment:', enrollment?.id);
    ...
    if (paymentMethod?.cheques && Array.isArray(cheques) && cheques.length && doc?.id) {
      try {
        ...
        console.log('✅ Cheques persisted, count:', cheques.length);
      } catch (e) {
