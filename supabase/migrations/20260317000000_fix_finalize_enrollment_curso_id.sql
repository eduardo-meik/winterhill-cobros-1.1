-- Migration: Fix column reference es.curso_id → s.curso in finalize_enrollment
-- Problem: The function references es.curso_id on the enrollment_students table,
--          but that column does NOT exist. The actual course UUID lives in
--          students.curso (uuid FK → cursos.id).
-- Fix:     JOIN students s ON s.id = es.student_id and use s.curso instead.
-- Affects: Student MAXIMILIANO AUGUSTO LOPEZ ORTIZ (8° BASICO B) and any other
--          enrollment confirmation that hits this code path.
-- ============================================================================
...
BEGIN
  ...
  -- FIX: removed es.curso_id (column does not exist on enrollment_students)
  FOR r_es IN
  ...
  -- FIX: replaced es.curso_id with s.curso via JOIN with students
  FOR r_es IN
    SELECT es.student_id, s.curso AS curso_id
      FROM public.enrollment_students es
  ...
  IF NOT v_dry_run THEN
    ...
    -- Also update students without curso in students table (legacy path)
    -- FIX: replaced es.curso_id IS NULL with s.curso IS NULL via JOIN
    UPDATE public.students
