-- CORRECTED finalize_enrollment function
-- Fixes the 400/409 errors by:
-- 1. Correcting ON CONFLICT to match actual constraint (student_id, guardian_id, numero_cuota)
-- 2. Adding folio generation
-- 3. Returning folio to enable receipt generation
...
DECLARE
  ...
  v_folio text; -- NEW: for folio generation
  r_es RECORD;
  r_cuota RECORD;
BEGIN
    ...
    BEGIN
      ...
      LOOP
        ...
        ELSE
          ...
          -- FIXED: Use correct constraint columns (student_id, guardian_id, numero_cuota)
  ...
  IF NOT v_dry_run THEN
    ...
    -- Set status to PRE_MATRICULADO instead of MATRICULADO
  ...
  RETURN jsonb_build_object(
    ...
    'details', v_summary,
    'folio', v_folio, -- NEW: Return folio
    'year', v_year
  );
END;
$$;

COMMENT ON FUNCTION public.finalize_enrollment IS 'Finalize enrollment: creates fees, generates folio, sets student status to PRE_MATRICULADO. Use dry_run=true to preview.';
